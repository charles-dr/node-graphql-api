...
const BrandCategoryModel = require(path.resolve('src/model/BrandCategoryModel'));
...
const activity = {
  translate: async () => {
    ...
    const total = await BrandCategoryModel.countDocuments(query); console.log('[Translation][BrandCategory][Total]', total);
    ...
    for (let i = 0; i < nIter; i++) {
      const brands = await BrandCategoryModel.find(query, null, { limit: batch, skip: 0 });
      logger.info(`[Translation][BrandCategory] [${i * batch} - ${(i + 1) * batch}] Start! [${brands.length}]`);
      await activity.processBatch(brands);
      logger.info(`[Translation][BrandCategory] [${i * batch} - ${(i + 1) * batch}] Done!`);
    }
  
    logger.info(`[Translation][BrandCategory] All Done!`);
